...
class KalmanFilter:
    ...
    def __init__(self, markers, initial_state):
        ...
        self.last_time = None    # keep track of time between measurements
    ...
    def prediction(self, v, imu_meas):
        ...
        imu_meas - a 5 by 1 numpy array consistening of the values
            (acc_x,acc_y,acc_z,omega,time), with the fourth of the
            values giving the gyroscope measurement for angular
            velocity (which you should use as ground truth) and time
            giving the current timestamp. Ignore the first three
            values (they are for the linear acceleration which we
            don't use)
        ...
        dfdx = np.eye(3) + dt * np.array([(0, 0, -v * np.sin(theta)),
                                          (0, 0, v * np.cos(theta)),
                                          (0, 0, 0)])
        dfdn = dt * np.array(((math.cos(theta), 1),
                              (math.sin(theta), 1), (1, 1)))
        xp = self.x + dt * \
            np.array((v * np.cos(theta), v * np.sin(theta), omega))
        Pp = np.dot(np.dot(dfdx, self.P_t), dfdx.T) + \
            np.dot(np.dot(dfdn, self.Q_t), dfdn.T)
    ...
    def transformMeasurement(self, meas):
        ...
        for tagNum in range(0, numberOfTags):
            ...
            # weight this measurement more heavily if theta is small
            weight = 1 / abs(theta)
            weightsSum += weight
            # the robot state according to apriltag measurement
            z_t = z_t + np.append(robotPos, robotTheta) * weight
